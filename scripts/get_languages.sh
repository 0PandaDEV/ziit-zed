...
fetch_files() {
    local repo_owner=$1
    local repo_name=$2
    local response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer ${GITHUB_TOKEN}" "https://api.github.com/repos/$repo_owner/$repo_name/git/trees/HEAD?recursive=1")
    local body=${response::-3}
    local status=${response: -3}

    if [[ "$status" != "200" ]]; then
        echo "Error fetching tree for $repo_owner/$repo_name: HTTP $status" >&2
        echo ""
        return
    fi

    echo "$body" | jq -c -r '.tree? // [] | .[].path'
}
...
process_language_files() {
    ...
    for file in $files; do
        ...
        local raw_url="https://raw.githubusercontent.com/$repo_owner/$repo_name/HEAD/${file}"
        local name=$(curl -s "$raw_url" | grep -oP '^name = "\K[^"]*')
        local id="${file%/config.toml}"
        id="${id##*/}"
...
# Fetch main repo files with error handling
RESPONSE=$(curl -s -w "%{http_code}" -H "Authorization: Bearer ${GITHUB_TOKEN}" "https://api.github.com/repos/zed-industries/zed/git/trees/main?recursive=1")
RESPONSE_BODY=${RESPONSE::-3}
RESPONSE_STATUS=${RESPONSE: -3}

if [[ "$RESPONSE_STATUS" != "200" ]]; then
    echo "Failed to fetch zed-industries/zed repo tree: HTTP $RESPONSE_STATUS" >&2
    FILES=""
else
    FILES=$(echo "$RESPONSE_BODY" | jq -c -r '.tree? // [] | .[].path')
fi
